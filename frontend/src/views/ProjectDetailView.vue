...
<template>
  <div>
    ...
    <template v-else-if="projectsStore.currentProject">
      <div class="mb-8">
        <RouterLink to="/projects" class="text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 flex items-center mb-4">
        ...
        <div class="flex items-center justify-between">
          <div class="flex items-center">
            ...
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ projectsStore.currentProject.name }}</h1>
          </div>
        ...
        <p v-if="projectsStore.currentProject.description" class="text-gray-600 dark:text-gray-400 mt-2">
      ...
      <div v-else-if="boardsStore.boards.length === 0" class="card p-12 text-center">
        ...
        <h3 class="mt-4 text-lg font-medium text-gray-900 dark:text-white">Nenhum quadro</h3>
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div v-for="board in boardsStore.boards" :key="board.id" class="card hover:shadow-md transition-shadow">
          <RouterLink :to="`/boards/${board.id}`" class="block p-6">
            <h3 class="font-semibold text-gray-900 dark:text-white">{{ board.name }}</h3>
            <p v-if="board.description" class="mt-2 text-sm text-gray-600 dark:text-gray-400 line-clamp-2">{{ board.description }}</p>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white dark:bg-dark-800 rounded-xl shadow-xl w-full max-w-md mx-4">
        <div class="p-6 border-b border-gray-100 dark:border-dark-600">
          <h2 class="text-xl font-semibold text-gray-900 dark:text-white">Novo Quadro</h2>
        </div>
